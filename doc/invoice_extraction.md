...
The script reads `data/source/requirements.yml` which defines the full invoice schema (parties, dates, payment, VAT breakdown, line items, etc.). Each image is sent to `gemini-3-flash-preview` along with the schema, and the model returns structured JSON matching the specification.
